...
# -----------------------
class BPlusTree:
    def __init__(self, order: int = 4, allow_duplicates: bool = False,leaf_capacity = 3):
        if order < 3:
    ...
    def insert_steps(self, key, value=None) -> Generator[Step, None, None]:
        ...
        total = len(leaf.keys)
        split_index = math.floor(total / 2)
    ...
    def delete_steps(self, key) -> Generator[Step, None, Optional[bool]]:
        ...
        min_keys = max(1, math.ceil(self.leaf_capacity/2))  # root can have fewer, but non-root must have at least this many
        ...
        idx = 0 
        while True:
            min_keys = min_keys if idx ==0 else math.ceil(self.order/2 -1)
            # if idx >0 :
...
with st.sidebar:
    ...
    with st.form("op_form"):
        leaf_capacity = st.number_input(
        "Leaf capacity (max keys per leaf)",
        min_value=2, max_value=10, value=3, step=1,
        help="Maximum number of keys a leaf node can hold before splitting."
        )
        order = st.number_input("Tree order (max children per internal node)",
                                min_value=3, max_value=11, value=4, step=1,
                                help="Order m: max children per internal node. Max keys per node = m-1")
        # st.selectbox("Tree order (m)",
        #                      options=[3, 4, 5, 6, 7, 8],
        #                      index=1,
        #                      help="Order m: max children per internal node. Max keys per node = m-1",
        #                      key="order_select")
