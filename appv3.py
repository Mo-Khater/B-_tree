...
def tree_to_snapshot(root: Optional[BPlusNode]) -> Dict[str, Any]:
    ...
    if root:
        rec(root)
    return {"root": root.id if root else None, "nodes": nodes}
...
class BPlusTree:
    ...
    def _make_step(self, action, description, detailed="", pseudocode_line=None,
        ...
        return Step(
            ...
            tree_snapshot=tree_to_snapshot(self.root),
    ...
    def insert_steps(self, key, value=None) -> Generator[Step, None, None]:
        ...
        while parent_index >= 0:
            ...
            if len(parent.keys) <= self.max_keys:
                ...
                mid = total // 2  # promote parent.keys[mid]
    ...
    def delete_steps(self, key) -> Generator[Step, None, Optional[bool]]:
        ...
        min_keys = max(1, self.min_children - 1)  # root can have fewer, but non-root must have at least this many
...
def render_tree_dot(snapshot: Dict[str, Any], highlight_nodes: List[str], highlight_keys: List[Tuple[str, int]]) -> str:
    """
    Build a DOT source string that uses HTML-like labels (tables) to show keys as cells.
    Colors:
      - leaf nodes: green-ish
      - internal nodes: lightblue
      - current node: yellow border
      - compared keys: red cell bgcolor
      - promoted/borrowed keys: blue cell bgcolor
    """
    ...
    # Helper to see which keys are highlighted
    highlight_map = {}
    ...
    for nid, data in nodes.items():
        keys = data["keys"]
        is_leaf = data["leaf"]
        bgcolor = "#b9f6ca" if is_leaf else "#b3e5fc"  # leaf green, internal blue
        # if node highlighted as current node, set border color and thicker pen
        border_style = ""
        if nid in highlight_nodes:
            border_style = 'style="rounded,filled"'
            # We'll indicate current node by extra yellow background around node table by outer table cell
            outer_bg = "#fff59d"
        else:
            outer_bg = bgcolor

        # Build inner row cells for keys
        cell_tds = []
        for i, k in enumerate(keys):
            cell_bg = "white"
            # comparisons: red, promotions/borrow: blue (we assume those passed in highlight_keys)
            if nid in highlight_map and i in highlight_map[nid]:
                # We can't know 'type' of highlight from snapshot, but UI sets highlight_keys for comparisons/promotions appropriately
                # We'll make it red (compare) or blue (meta could specify) — but we only have (nid,i) so default to red
                cell_bg = "#ffcdd2"  # light red
            # Escape key for HTML
            cell_tds.append(f'<td port="k{i}" bgcolor="{cell_bg}" border="0" cellspacing="0" cellpadding="6">{html.escape(str(k))}</td>')
        # If no keys (possible for empty root?), add empty cell
        if len(cell_tds) == 0:
            cell_tds_html = '<td bgcolor="white" cellpadding="6"> </td>'
        else:
            cell_tds_html = "".join(cell_tds)

        # Label as an HTML-like table: outer table for node, inner row for keys
        label = f'''<
    ...
    # now add edges: internal node children edges, and leaf next pointers as dotted arrows
    for nid, data in nodes.items():
        if not data["leaf"]:
            # children are node IDs
            for i, child_id in enumerate(data["children"]):
                dot_lines.append(f'  {nid} -> {child_id} [arrowhead=none];')

    # add leaf pointers (next)
    for nid, data in nodes.items():
